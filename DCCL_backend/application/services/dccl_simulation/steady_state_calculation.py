...
def main_cavity_trans(U_pre, H_fsf, B_CatEye1, B_aper, B_CatEye2, lm,rm,sigma_medium,tau_medium,P_in,lambda_,eta_c):
    # lm = 0.001  # 增益介质长度
    U = cal_field_transition(U_pre, H_fsf, B_CatEye1, B_aper)  # M1-L1入射表面
    U = propagation_within_gain(U, lm,rm,sigma_medium,tau_medium, P_in, lambda_,eta_c)
...
def one_roundtrip_distribution(U_M1pre, U_M2pre, matrix_all,aperture_all,data,lambda_):
    ...
    sigma_medium = data['modelAttributeList'][2]['sigma']  # 增益介质吸收截面
    tau_medium = data['modelAttributeList'][2]['tau']  # 增益介质寿命
    U_gain1, U5 = process_main_cavity(U_M1pre,  matrix_all[1], aperture_all[0], aperture_all[1], aperture_all[2],lm,rm,sigma_medium,tau_medium, P_in, lambda_, eta_c, r2, t2)
    # 第二阶段：自由腔传输
    # log_gpu_memory("222")
    ...
    del U_gain2
    # log_gpu_memory("333")
    U_M1 = process_gain_and_merge(U_gain1 ,matrix_all[1], aperture_all[1], aperture_all[0], r1, lm,rm, sigma_medium,tau_medium,P_in, lambda_, eta_c)
    del U_gain1
    # 显存清理
    # cp.get_default_memory_pool().free_all_blocks()
    return U_M1, U, U2
    
def process_main_cavity(U_M1pre, H_fsf, B_CatEye1, B_aper, B_CatEye2,lm,rm,sigma_medium,tau_medium, P_in, lambda_, eta_c, r2, t2):
    U1 = main_cavity_trans(U_M1pre, H_fsf, B_CatEye1, B_aper, B_CatEye2, lm,rm,sigma_medium,tau_medium,P_in, lambda_, eta_c)
...
def process_gain_and_merge(U_gain1, H_fsf, B_aper, B_CatEye1, r1, lm,rm,sigma_medium,tau_medium, P_in, lambda_, eta_c):
    U_combined = U_gain1
    U_combined = propagation_within_gain(U_combined, lm,rm,sigma_medium,tau_medium, P_in, lambda_, eta_c)
